...
incus init images:ubuntu/jammy/cloud ntp-make -t c2-m6 --network dns-infra -d eth0,ipv4.address=192.168.47.50 -d root,size=320GiB
incus init images:ubuntu/jammy/cloud ntp-client -t c2-m6 --network dns-infra -d eth0,ipv4.address=192.168.47.101 -d root,size=320GiB
...
# Setup ntp-client (DNS Server)
echo "========== Setting up ntp-client (DNS Server) =========="
incus exec ntp-client -- /bin/bash -c "apt update && sleep 5 && DEBIAN_FRONTEND=noninteractive apt-get install -y net-tools bind9 bind9utils bind9-doc dnsutils tshark"

# Ensure bind9 directories exist
incus exec ntp-client -- /bin/bash -c "mkdir -p /etc/bind/zones"

# Enable and start bind9
incus exec ntp-client -- systemctl enable --now bind9

# Setup ntp-make (DNS Client)
echo "========== Setting up ntp-make (DNS Client) =========="
incus exec ntp-make -- /bin/bash -c "apt update && sleep 5"
...
incus exec ntp-make -- /bin/bash -c "DEBIAN_FRONTEND=noninteractive apt-get install -y dnsutils tshark net-tools"

# Allow ansible user to run tshark without root privileges
incus exec ntp-make -- /bin/bash -c "usermod -aG wireshark ansible"

# Push DNS configuration files
echo "========== Configuring DNS on ntp-client =========="
incus file push ../named.conf.options ntp-client/etc/bind/named.conf.options
incus file push ../named.conf.local ntp-client/etc/bind/named.conf.local
incus file push ../research.net ntp-client/etc/bind/zones/research.net
incus file push ../research.net.rev ntp-client/etc/bind/zones/research.net.rev
incus file push ../resolv.conf ntp-client/etc/resolv.conf

# Restart bind9 to apply changes
incus exec ntp-client -- systemctl restart bind9
incus exec ntp-client -- systemctl status bind9 --no-pager

# Set resolv.conf on ntp-make
